...
export function evaluateCompatibility(
  ...
  if (!cat) return { isCompatible: true, reasons: [] };

  const mockBuild = { ...currentBuild };
  
  // Resolve correct key name (e.g. support indexed keys like 'ram_0', 'storage_1')
  let targetKey: string = cat;
  const existingKey = Object.keys(mockBuild).find(k => {
    if (cat === 'ram') return isRamKey(k);
    if (cat === 'storage') return isStorageKey(k);
    if (cat === 'gpu') return isGpuKey(k);
    if (cat === 'fan') return isFanKey(k);
    return k === cat;
  });
  ...
  mockBuild[targetKey] = component;

  // Run the full validation suite
  const result = validateCompatibility(mockBuild);

  // Filter issues to those associated with the category of the component being evaluated
  const categoryIssues = [...result.errors, ...result.warnings].filter(issue => 
    issue.components.includes(cat) || 
    (cat === 'psu' && (issue.rule === 'psu_underpowered' || issue.rule === 'psu_tight'))
  );

  // Check if any category-specific issues represent validation errors (warnings don't block compatibility status)
  const hasErrors = categoryIssues.some(issue => 
    result.errors.some(e => e.rule === issue.rule)
  );
  
  const reasons = categoryIssues.map(issue => issue.message);

  return { isCompatible: !hasErrors, reasons };
}