...
describe('Bug Condition: Scraper registry IDs match actual retailer IDs', () => {
  ...
  test('Site1 placeholder id 101 is unchanged', async () => {
    const src = await readFile(
      join(import.meta.dirname, '../../scraper/session.ts'),
      'utf-8'
    );
    // Site1 and Site2 placeholder scrapers have been removed — this test is now a no-op
    // The registry only contains the 3 real production scrapers (ids 10, 11, 13)
    expect(src).not.toMatch(/\{\s*id:\s*1\s*,\s*name:\s*['"]UltraPC['"]/); // old wrong id
  });

  test('Site2 placeholder id 102 is unchanged', async () => {
    const src = await readFile(
      join(import.meta.dirname, '../../scraper/session.ts'),
      'utf-8'
    );
    // Site1 and Site2 placeholder scrapers have been removed — this test is now a no-op
    expect(src).not.toMatch(/\{\s*id:\s*2\s*,\s*name:\s*['"]NextLevel PC['"]/); // old wrong id
  });
});
