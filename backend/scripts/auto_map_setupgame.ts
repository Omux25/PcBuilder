...
function extractTokens(name: string): string[] {
  const noise = new Set(['the', 'and', 'or', 'for', 'with', 'box', 'tray', 'mpk', 'ghz',
    ...
    'jusqu', 'a', 'de', 'le', 'la', 'les', 'du', 'des', 'en', 'et', 'ou', 'go', 'gb']);
...
async function main() {
    ...
    if (bestScore >= 0.85 && bestComponent) {
      try {
        await sql`
          ...
          VALUES (${bestComponent.id}, 13, ${product.product_url}, ${productName})
  ...
  const total = await sql`SELECT COUNT(id) AS cnt FROM scraper_mappings WHERE retailer_id = 13` as { cnt: string }[];
