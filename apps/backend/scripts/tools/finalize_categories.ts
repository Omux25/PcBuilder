...
async function finalCategorization() {
    ...
    if (bundlesInPsu.length > 0) {
        ...
        await sql`UPDATE components SET category = 'case' WHERE id IN ${sql(bundlesInPsu.map((r: any) => r.id))}`;
    }
    ...
    if (casesInCooling.length > 0) {
        ...
        await sql`UPDATE components SET category = 'case' WHERE id IN ${sql(casesInCooling.map((r: any) => r.id))}`;
    }
