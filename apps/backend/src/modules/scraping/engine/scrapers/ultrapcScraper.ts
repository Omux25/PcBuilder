...
export class UltraPcScraper extends BaseScraper {
  ...
  protected extractPrices($: CheerioAPI): ScrapedPrice[] {
    ...
    $('article.product-miniature, .js-product-miniature').each((_i, card) => {
      ...
      // Image — collect all available images in the card
      const cardImages = new Set<string>();
      $(card).find('img').each((_i, imgEl) => {
        const src = $(imgEl).attr('data-full-size-image-url') ??
                    $(imgEl).attr('data-src') ??
                    $(imgEl).attr('src');
        if (src) {
          const cleanSrc = src.replace(/-large_banner\//, '-large_default/')
                             .replace(/-home_default\//, '-large_default/');
          cardImages.add(cleanSrc);
        }
      });
      const image_urls = Array.from(cardImages).slice(0, 3);
