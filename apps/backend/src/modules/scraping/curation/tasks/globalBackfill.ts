...
export async function globalBackfill(sql: SqlFn): Promise<TaskResult> {
  ...
  if (missing.length === 0) {
    return {
      success: true,
      mutatedCount: 0,
      message: 'All GPUs have physical dimensions, no knowledge healing required.'
    };
  }
  ...
  return {
    success: true,
    mutatedCount: healedCount,
    message: `Healed specs and clearances for ${healedCount} GPU(s).`
  };
}
