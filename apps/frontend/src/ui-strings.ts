...
export const UI = {
  ...
  compare: {
    ...
    subtitle: (max: number) => `Comparez jusqu'à ${max} composants côte à côte.`,
    duelTitle: 'Duel de Performance',
    noData: 'Données de performance insuffisantes pour un duel.',
    duelWinner: (winner: string, diff: string, loser: string) =>
      `${winner} est ${diff}% plus performant que ${loser}`,
    specLabel: 'Spécification',
    sectionPrice: 'Prix',
    sectionPerf: 'Analyse de Performance',
    ...
    perfPerPriceHint: "Plus c'est haut, mieux c'est",
    removeTitle: 'Retirer de la comparaison',
    addComponent: 'Ajouter un composant',
    add: 'Ajouter',
    browse: 'Parcourir →',
    emptyTitle: 'Prêt pour un duel ?',
    emptyText: 'Ajoutez des composants depuis le catalogue pour comparer leurs caractéristiques et performances.',
  ...
  trends: {
    ...
    subtitle: 'Analyse des variations de prix réelles pour les produits en stock.',
    drops: 'Bons Plans (Baisses)',
    hikes: 'Inflations (Hausses)',
    period: 'Période',
    category: 'Catégorie',
    allCategories: 'Toutes',
    dayUnit: (d: number) => `${d}j`,
    resultCount: (n: number, days: number) =>
      `${n} variation${n > 1 ? 's' : ''} détectée${n > 1 ? 's' : ''} sur ${days} jours`,
    noResults: (days: number) =>
      `Aucune variation significative détectée sur les ${days} derniers jours.`,
    retry: 'Réessayer',
    savings: 'Économie',
    hike: 'Hausse',
