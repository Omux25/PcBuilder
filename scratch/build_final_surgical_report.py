...
def build_final_surgical_report():
    ...
    doc = Document()
    print("Building FINAL SURGICAL REPORT (Manual Review applied)...")
    ...
    # Surgical Content Mapping
    sections = [
        ("Remerciements", lines[11:17]),
        ("Résumé", lines[18:22]),
        ("Abstract", lines[23:25]),
        ("Table des matières", []),
        ("Introduction Générale", lines[27:34]), # REMOVED duplicate trailer (lines 52-55)
        ...
        ("2.3 Solution Proposée", lines[90:92]), # STOPPED at 92 to avoid stray Chapitre 3 header
        ("2.4 Planning", []),
        ...
        ("4.5 Interface utilisateur", lines[145:151]), # STOPPED at 151 to avoid duplicate Figure 9 caption
    ...
    for title, content in sections:
        ...
        for line in content:
            ...
            # REMOVE STRAY FIGURE CAPTIONS FROM SOURCE TEXT
        ...
        if "Planning" in title: 
            ...
            doc.add_paragraph(f"Figure {fig_count} : Planning").alignment = WD_ALIGN_PARAGRAPH.CENTER
            fig_count += 1
    ...
    doc.save(final_path)
    print("Final Surgical Report Built.")

if __name__ == "__main__":
    build_final_surgical_report()
