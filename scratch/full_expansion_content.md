# PFA Report: Massive Expansion Content
...
3. **Hiérarchie de l'Information :** Les specs critiques (Socket, Wattage, Prix) sont mises en évidence par des badges colorés, permettant une lecture rapide.
...
1. **Socket :** Le socket du CPU doit figurer dans la liste des sockets supportés par la carte mère.
2. **TDP :** La somme des consommations crêtes ne doit pas dépasser 80% de la capacité de l'alimentation.
3. **Dimensions :** La longueur du GPU est comparée à la "Max GPU Length" du boîtier.
4. **RAM :** Le type de slot (DDR4 vs DDR5) doit correspondre strictement.

## Section 4: Manuel de l'Utilisateur (Annexes)
1. **Sélection du Processeur :** Commencez toujours par le CPU, car il dicte le choix de la carte mère.
...
- **Gestion des Scrapers :** Les collecteurs peuvent être déclenchés individuellement depuis le dashboard.
- **Mapping Manuel :** Une interface dédiée permet de lier les produits "Inconnus" en un clic.
- **Logs Techniques :** En cas de changement de structure HTML chez un revendeur, le système génère une alerte visuelle pour maintenance immédiate du collecteur CSS.
