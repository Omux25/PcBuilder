...
def build_tech_french_report():
    ...
    doc = Document()
    print("Building TECH-HYBRID FRENCH REPORT...")
    ...
    sections = [
        ...
        ("Résumé", lines[17:21]),
        ("Résumé Technique (Abstract)", french_abstract),
        ("Table des matières", []),
        ...
        ("3.1 Choix Techniques et Stack", lines[91:96]), # ADDED STACK
        ...
        ("4.3 Moteur de compatibilité et DNA Matcher", lines[119:126] + extract_md_section("## Section 3")), # REVERTED
        ("4.4 Système de Scraping automatique", lines[128:135]), # REVERTED
        ("4.5 Interface utilisateur (Front-end)", lines[137:143]), # REVERTED
        ("4.6 Dashboard d'administration", lines[145:151]), # REVERTED
        ("Chapitre 5 : Résultats, tests et validation", []),
        ("5.1 Matrice de Tests Détaillée", [
            "ID | Fonctionnalité | Scénario | Résultat",
            "TC-01 | Authentification | Login Admin | Succès",
            "TC-02 | Compatibilité | Vérification Socket | Succès",
            "TC-03 | Scraper | Extraction de prix | Succès",
            "TC-04 | DNA Matcher | Score de correspondance | Succès",
            "TC-05 | UI | Layout Responsive | Succès"
        ]),
    ...
    for title, content in sections:
        ...
        if "Interface utilisateur" in title: add_fig(os.path.join(assets_dir, "figure_7_home_1778642725154.png"), "Dashboard du Configurateur")
    ...
    doc.save(final_path)
    print("Massive Tech-Hybrid Report Built.")

if __name__ == "__main__":
    build_tech_french_report()
